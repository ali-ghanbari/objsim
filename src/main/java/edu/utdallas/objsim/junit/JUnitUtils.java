package edu.utdallas.objsim.junit;
...
public class JUnitUtils {
    ...
    public static List<CloseableTestUnit> discoverTestUnits(final Collection<String> classNames) {
        final List<CloseableTestUnit> testUnits = new LinkedList<>();
        ...
        VISITED.clear();
        Collections.sort(testUnits, createTestUnitComparator());
        return testUnits;
    }
    ...
    private static boolean shouldAdd(final Class<?> testClass, final Method testMethod) {
        Set<Method> methods = VISITED.get(testClass);
        if (methods == null) {
            methods = new HashSet<>();
            VISITED.put(testClass, methods);
        }
        return methods.add(testMethod);
    }

    private static Collection<CloseableTestUnit> findJUnit3XXTestUnits(Collection<Class<?>> classes) {
        final List<CloseableTestUnit> testUnits = new LinkedList<>();
    ...
    private static Collection<CloseableTestUnit> findJUnit3XXTestUnits(final Class<?> testClass) {
        final List<CloseableTestUnit> testUnits = new LinkedList<>();
    ...
    private static Collection<CloseableTestUnit> findJUnit4YYTestUnits(Collection<Class<?>> classes) {
        final List<CloseableTestUnit> testUnits = new LinkedList<>();
        for (final Class<?> clazz : classes) {
            if (isAbstract(clazz)) {
                continue;
    ...
    private static CloseableTestUnit createTestUnit(final Class<?> testClass,
        ...
        return new WrappingTestUnit(new AdaptedJUnitTestUnit(testClass, Option.some(filter)));
    }
